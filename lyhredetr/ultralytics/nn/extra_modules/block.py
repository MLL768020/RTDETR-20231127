...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'C3_Faster', 'C2f_Faster', 'C3_Faster_EMA', 'C2f_Faster_EMA', 'C3_Faster_Rep', 'C2f_Faster_Rep',
           'C3_Faster_Rep_EMA', 'C2f_Faster_Rep_EMA',
           'BasicBlock_PConv', 'BottleNeck_PConv', 'BasicBlock_PConv_Rep', 'BottleNeck_PConv_Rep',
           'BasicBlock_Faster_Block', 'BottleNeck_Faster_Block',
           'BasicBlock_Faster_Block_EMA', 'BottleNeck_Faster_Block_EMA', 'BasicBlock_Faster_Block_Rep',
           'BottleNeck_Faster_Block_Rep', 'BasicBlock_Faster_Block_Rep_EMA', 'BottleNeck_Faster_Block_Rep_EMA',
           'C3_AKConv', 'C2f_AKConv', 'BasicBlock_AKConv', 'BottleNeck_AKConv', 'AKConv', 'RFAConv', 'RFCBAMConv',
           'RFCAConv',
           'C3_RFAConv', 'C2f_RFAConv', 'C3_RFCBAMConv', 'C2f_RFCBAMConv', 'C3_RFCAConv', 'C2f_RFCAConv',
           'BasicBlock_RFAConv', 'BottleNeck_RFAConv', 'BasicBlock_RFCBAMConv', 'BottleNeck_RFCBAMConv',
           'BasicBlock_RFCAConv', 'BottleNeck_RFCAConv',
           ...
           'DWRC3_DRB', 'C3_DWR_DRB', 'C2f_DWR_DRB', 'Conv3XCC3', 'DRBC3', 'DBBC3', 'C2f_DBB', 'C3_DBB',
           'BasicBlock_DBB', 'BottleNeck_DBB', 'CSP_EDLAN','DBSPP',
           'BasicBlock_DualConv', 'BottleNeck_DualConv', 'Zoom_cat', 'ScalSeq', 'DynamicScalSeq', 'Add',
           'asf_attention_model',
           ...
           'C3_SWC', 'C2f_SWC', 'BasicBlock_SWC', 'BottleNeck_SWC', 'C3_iRMB_DRB', 'C2f_iRMB_DRB',
           'BasicBlock_iRMB_DRB', 'BottleNeck_iRMB_DRB',
           ...
           'ContextGuidedBlock_Down', 'BasicBlock_ContextGuided', 'BottleNeck_ContextGuided', 'C3_ContextGuided',
           'C2f_ContextGuided',
           'C2f_DAV'
           ]
...
class BottleNeck_Ortho(nn.Module):
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', height=64, variant='d'):
        ...
        self._excitation = nn.Sequential(
            nn.Linear(in_features=ch_out * self.expansion, out_features=round(ch_out / 16 * self.expansion),
                      bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(in_features=round(ch_out / 16 * self.expansion), out_features=ch_out * self.expansion,
                      bias=False),
            nn.Sigmoid(),
        )
        self.OrthoAttention = Attention_Ortho()
...
class C3_Ortho(C3):
    def __init__(self, c1, c2, n=1, height=16, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.Sequential(
            *(Bottleneck_Ortho(c_, c_, shortcut, g, k=(1, 3), e=1.0, height=height) for _ in range(n)))
...
class C2f_Ortho(C2f):
    def __init__(self, c1, c2, n=1, height=16, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(
            Bottleneck_Ortho(self.c, self.c, shortcut, g, k=(3, 3), e=1.0, height=height) for _ in range(n))
...
class Conv3XC(nn.Module):
    ...
    def switch_to_deploy(self):
        if not hasattr(self, 'eval_conv'):
            ...
            weight_concat = F.conv2d(w.flip(2, 3).permute(1, 0, 2, 3), w3, padding=0, stride=1).flip(2, 3).permute(1, 0,
                                                                                                                   2, 3)
            ...
            self.eval_conv = nn.Conv2d(in_channels=self.inc, out_channels=self.ouc, kernel_size=3, padding=1,
                                       stride=self.sk.stride, bias=self.bias)
...
######################################## Dilation-wise Residual end ########################################
# llf
class Bottleneck_DAV(Bottleneck):
...
class C2f_DAV(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_DAV(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
...
class DBSPP(nn.Module):

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(DBSPP, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1, g=4)
        self.cv2 = Conv(c1, c_, 1, 1, g=4)
        self.cv3 = DiverseBranchBlock(c_, c_, 3, 1, groups=4)
        self.cv4 = Conv(c_, c_, 1, 1, g=4)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.cv5 = Conv(4 * c_, c_, 1, 1, g=4)
        self.cv6 = DiverseBranchBlock(c_, c_, 3, 1, groups=4)
        self.cv7 = Conv(4 * c_, c2, 1, 1, g=4)
        self.mean = nn.AdaptiveAvgPool2d((1, 1))
        self.cv8 = Conv(c1, c_, 1, 1, g=4)
    ...
    def forward(self, x):
        ...
        y3 = self.mean(x)
        y3 = self.cv8(y3)
        y3 = torch.nn.functional.interpolate(y3, size=size, mode='bilinear')

        return self.cv7(torch.cat((y1, y1, y2, y3), dim=1))
