...
# yhloss_1
def yhloss_1(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, yhloss1=False,
             pyhloss1=False, gyhloss1=False, eps=1e-7,
             ratio=0.7, Lambda=1.3):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    ...
    # IoU
    iou = inter / union
    if CIoU or DIoU or GIoU or EIoU or SIoU or yhloss1 or pyhloss1 or gyhloss1:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or EIoU or SIoU or yhloss1 or pyhloss1 or gyhloss1:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            c2 = cw ** 2 + ch ** 2 + eps  # convex diagonal squared
            rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4  # center dist ** 2
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi ** 2) * (torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2)
                ...
                cw2 = cw ** 2 + eps
                ch2 = ch ** 2 + eps
                cp = rho2  # centerpoint(b,b_gt)
                x1_min = torch.max(b1_x1, b2_x1)
                y1_min = torch.max(b1_y1, b2_y1)
                x2_max = torch.min(b1_x2, b2_x2)
                y2_max = torch.min(b1_y2, b2_y2)

                # 如果交叉区域存在
                if (x2_max > x1_min).all() and (y2_max > y1_min).all():
                    # Your code here

                    # 计算交叉区域的对角线长度的平方
                    diagonal_length_squared = (x2_max - x1_min) ** 2 + (y2_max - y1_min) ** 2
                else:
                    # 如果交叉区域不存在，则对角线长度的平方为 0
                    diagonal_length_squared = 0.0

                isp = diagonal_length_squared  # intersectionpoint
                impact = cp / (cp + isp)
                d1 = (b2_x1 - b1_x1) ** 2 + (b2_y1 - b1_y1) ** 2
                d2 = (b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2
                d = torch.sqrt(d1 * d2)
                ...
                return innner_iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2 + gamma * impact)
            elif pyhloss1:
                rho_w2 = ((b2_x2 - b2_x1) - (b1_x2 - b1_x1)) ** 2
                rho_h2 = ((b2_y2 - b2_y1) - (b1_y2 - b1_y1)) ** 2
                cw2 = cw ** 2 + eps
                ...
                q = torch.exp(-P)
                x = q * Lambda
                return innner_iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2 + beta * impact)
            elif SIoU:
                # SIoU Loss https://arxiv.org/pdf/2205.12740.pdf
                s_cw = (b2_x1 + b2_x2 - b1_x1 - b1_x2) * 0.5 + eps
                s_ch = (b2_y1 + b2_y2 - b1_y1 - b1_y2) * 0.5 + eps
                ...
                if torch.all(beta < 2):
                    ...
                    gamma = 2
                c_area = cw * ch + eps
                return innner_iou - (c_area - union) / c_area - gamma * impact
            return innner_iou - rho2 / c2  # DIoU
        c_area = cw * ch + eps  # convex area
...
def bbox_sim_iou(box1, box2, xywh=True, mpdiou_hw=2, ratio=0.7, k=0.5, eps=1e-7):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    """
    ...
    omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
    shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
    return innner_iou - (0.75 * (d1 / mpdiou_hw + d2 / mpdiou_hw) + (1 - k) * shape_cost)  # MPDIoU


def bbox_IMS_iou(box1, box2, xywh=True, mpdiou_hw=2, ratio=0.7, k=0.5, eps=1e-7):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    """
    ...
    omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
    shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
    return innner_iou - (1.25 * (d1 / mpdiou_hw + d2 / mpdiou_hw) + (k) * (shape_cost + distance_cost))  # MPDIoU


def bbox_new_iou(box1, box2, xywh=True, mpdiou_hw=2, ratio=0.7, eps=1e-7):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    """
    ...
    omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
    shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
    return innner_iou ** 2 - (d1 / mpdiou_hw + d2 / mpdiou_hw) ** 2 - v * alpha  # MPDIoU


def bbox_new1_iou(box1, box2, xywh=True, mpdiou_hw=2, ratio=0.7, eps=1e-7):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    """
    ...
    omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
    shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
    return innner_iou ** 1.2 - (d1 / mpdiou_hw + d2 / mpdiou_hw) ** 1.2 - 0.5 * v * alpha  # MPDIoU


def bbox_new2_iou(box1, box2, xywh=True, mpdiou_hw=2, ratio=0.7, eps=1e-7):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    """
    ...
    omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
    shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
    return innner_iou - (d1 / mpdiou_hw) ** 0.8 - (d2 / mpdiou_hw) ** 0.8 - (v * alpha)  # MPDIoU
...
class ConfusionMatrix:
    ...
    def plot(self, normalize=True, save_dir='', names=(), on_plot=None):
        ...
        sn.set(font_scale=2 if nc < 50 else 0.8)  # for label size
